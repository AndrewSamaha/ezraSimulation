...
export const isDNA = (obj: any): obj is DNA => {
  return (
    ...
    typeof obj === 'object' &&
    'visualSearch' in obj &&
    'stayEarningMultiplier' in obj &&
    'eatEarningMultiplier' in obj &&
    'energyGiftToOffspring' in obj &&
    'reproductionProbability' in obj &&
    'minimumEnergyToReproduce' in obj
  );
};
...
export const mutateDNA = (
  ...
  (Object.keys(newDNA) as Array<keyof DNA>)
    .filter((trait) => trait !== 'lineageName')
    .forEach((trait) => {
      ...
      if (Math.random() < mutationRate) {
        ...
        if (range) {
          newDNA[trait][randomIndex] = Math.max(
            range.min,
            Math.min(range.max, newDNA[trait][randomIndex]),
          );
        }
      }
