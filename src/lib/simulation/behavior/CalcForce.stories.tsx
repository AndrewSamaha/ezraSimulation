...
interface SimulationObject {
  ...
  energy: number;
  actionHistory: Array<unknown>;
}

// Create a mock simulation objec
const createMockObject = (
  ...
  energy: 100,
  actionHistory: [],
  dna: type === ObjectTypeEnum.ORGANISM ? (overrides.dna as DNA) || PLANT_DNA_TEMPLATE : undefined,
  ...overrides,
});
...
export const CalcForceSandbox = () => {
  ...
  const curObject = createMockObject(curObjectType, CENTER_POSITION, {
    ...
    <div className="flex flex-col h-screen bg-gray-900 text-white">
      ...
      <div className="p-4 bg-gray-800 border-b border-gray-700 overflow-x-auto">
        <div className="flex flex-wrap gap-6 text-sm">
          <div>
            <span className="font-semibold">Mouse:</span>({mousePosition.x.toFixed(0)},{' '}
            {mousePosition.y.toFixed(0)})
          </div>
          <div>
            <span className="font-semibold">Center:</span>({centerX.toFixed(0)},{' '}
            {centerY.toFixed(0)})
          </div>
          <div>
          ...
          <div>
            ...
            {forceMultiplier.toLocaleString()}
          </div>
          <div>
            <span className="font-semibold">Force Vector:</span>({forceVector.x.toFixed(6)},{' '}
            {forceVector.y.toFixed(6)})
          </div>
          <div>
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            height: '20px',
            backgroundColor:
              targetObjectType === ObjectTypeEnum.ORGANISM
                ? 'rgba(0, 100, 200, 0.5)'
                : 'rgba(200, 200, 0, 0.5)',
            transform: 'translate(-50%, -50%)',
            borderColor: 'white',
      ...
      <div className="p-4 bg-gray-800 border-t border-gray-700 text-sm">
        <p>
          Move your mouse to position the target object. The force vector shows the force acting on
          the current object.
        </p>
