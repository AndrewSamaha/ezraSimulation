...
export async function GET(
  ...
  try {
    ...
    const simulationStep = await db.query.simulationSteps.findFirst({
      where: and(eq(simulationSteps.simulationId, id), eq(simulationSteps.stepNumber, stepNumber)),
    });
    ...
    return NextResponse.json({ error: 'Failed to fetch simulation step' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to delete simulation step' }, { status: 500 });
  }
}
